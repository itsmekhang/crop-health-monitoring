...
\title{Multimodal Crop Health Monitoring:\\
...
% ── Abstract ──────────────────────────────────────────────────────────────────
\begin{abstract}
This paper documents the first full implementation of a modular crop health system that combines an image-based disease classifier with a weather-informed risk layer. A ResNet-18 model fine-tuned on PlantVillage classifies leaf disease across 15 classes covering Tomato, Potato, and Pepper. An XGBoost model then approximates a rule-based risk formula using the disease output and live weather conditions from the Open-Meteo API. Both models are integrated into a Streamlit dashboard that provides real-time diagnosis, environmental risk assessment, and estimated revenue loss. This report documents the implementation decisions, an honest evaluation of both models, and the key challenges identified during this phase, particularly class imbalance, the lab-to-field generalization gap, and the derived nature of the XGBoost training target.
\end{abstract}
...
\subsection{Purpose}
Farmers still rely on manual disease scouting or delayed laboratory testing, leading to late intervention and unnecessary crop losses. This project builds a modular AI prototype that processes a leaf photograph and a location, and returns a disease diagnosis, an environmental risk score, a treatment plan, and an estimated revenue impact. The goal is a functioning, honest prototype -- not an overclaimed production system.
...
\begin{table}[ht]
\caption{PlantVillage Class Distribution (local dataset)}
\label{tab:classes}
\centering
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Class} & \textbf{Images} & \textbf{Val~(\%)} \\
\midrule
Tomato\_YellowLeaf\_Curl\_Virus & 6{,}416 & 15.5 \\
Tomato\_Bacterial\_spot         & 4{,}254 & 10.3 \\
Tomato\_Late\_blight             & 3{,}817 & 9.2 \\
Tomato\_Septoria\_leaf\_spot    & 3{,}542 & 8.6 \\
Tomato\_Spider\_mites           & 3{,}352 & 8.1 \\
Tomato\_healthy                  & 3{,}182 & 7.7 \\
Pepper\_Bacterial\_spot         & 1{,}994 & 4.8 \\
Tomato\_mosaic\_virus           &   746 & 1.8 \\
Potato\_healthy                  &   304 & 0.7 \\
\midrule
\textbf{Total (15 classes)}     & \textbf{41{,}274} & -- \\
\bottomrule
\end{tabular}
\end{table}

The dataset is strongly imbalanced: the most frequent class (\textit{Tomato\_YellowLeaf\_Curl\_Virus}, 6{,}416 images) has 21$\times$ more samples than the least frequent (\textit{Potato\_healthy}, 304 images). Tomato accounts for 77.5\% of all images; Pepper and Potato account for 12.0\% and 10.4\%, respectively. Top-line validation accuracy therefore overestimates practical performance on minority classes.
